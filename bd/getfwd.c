...
//
// 16-Aug-1998
// demo, get records in key order, forward
//

#include "bullet_3.h"
#include "blt3demo.h"

#ifdef TEST_MODULE_GETFWD
 #define RECS_TO_ADD 25000
#endif

// ------------------------------------------------------
// Get records in key order, forward
...
TBLT_RETC DemoGetForward(TBLT_KH *khPtr, ULONG expected) {
 ...
 while (rc == 0) {
    ...
    if (rc == 0) {
       ...
       if (thisKey < lastKey) {
          printf("GetNext order is invalid (last key=0x%X, this key=0x%X, cnt=%lu)\n",lastKey, thisKey, cnt);
 ...
 if (rc == 0 && cnt != expected) {
    printf("Get Forward expected %lu, got %lu\n",expected,cnt);
    rc = 1;
 }
